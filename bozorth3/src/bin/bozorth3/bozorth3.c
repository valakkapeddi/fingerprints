...
int ignore_main() {
    ...
    while (1) {
        ...
        size_t current_args_len = 0;

        char* tok = strtok(input_line, delimiter);
        while (tok) {
            *(args + current_args_len++) = strdup(tok);
            tok = strtok(0, delimiter);
        }
        *(args + current_args_len) = 0;
...
int old_main(int argc, char ** argv) {
    ...
    while (1) {
        ...
        fprintf(stdout, "Fixed probe file is %d\n", fixed_probe_file);
